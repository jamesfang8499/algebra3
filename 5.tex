...
\begin{ex}
\begin{enumerate}
    \item 已知函数$y=f(x)=x^2-x+2$的图像上有一动点$Q(x,y)$及定点$P(2,4)$.
\begin{enumerate}[(1)]
...
\section{导数的几何意义}
在12.1节中我们已经知道，若点$Q(x_0+\Delta x,y_0+\Delta y)$、$P(x_0,y_0)$分别是函数$y=f(x)$的图像上的两点，则割线的斜率为
\[K_{PQ}=\frac{\Delta y}{\Delta x}=\frac{f(x_0+\Delta x)-f(x_0)}{\Delta x}\]
...
$y=f(x)$的图像无限地趋近于$P(x_0,y_0)$点，这时割线$PQ$绕着点$P$转动，它的极限位置是函数$y=f(x)$的图像在点$P$处的切线. 因此，
...
\begin{ex}
\begin{enumerate}
    \item 作函数$y=|x^2-x|$的图像，并说明它在哪个点不可导.
...
\end{split}\]
$\therefore\quad f'(x)=(x^n)'=nx^{n-1}$（$n$为自然数）.
    \item $f(x)=\sin x$的导数.
    \[\begin{split}
...
\begin{example}
求函数 $y= \sqrt [ 3] {\frac {x+ 2}{x- 1}}$ $( x> 1)$的导数
\end{example}
...
\textbf{解法2} $\because\quad y= \sqrt[3]{\frac{x+2}{x-1}} \quad (x>1)$
...
&=\left(3e^{3x}\sin2x+2e^{3x}\cdot \cos 2 x\right)\dd x
    \end{split}\]
\end{solution}
...
\item 若恒有$f(x)<f(x_0)$，则称函数$f(x)$在点$x_0$处有一极大值$f(x_0)$, $x_0$称为函数$f(x)$的一个极大值点.
\end{enumerate}
函数的极大值和极小值可统称为函数的极值.
\end{thm}


从图12.7和图12.8可看出，若$f(x)$在点$x_0$处有极值，且$f(x)$在点$x_0$可导，则函数$y=f(x)$的图像在点$(x_0,f(x_0))$处的切线平行于$x$轴. 即$f'(x_0)=0$.（证明从略）.
...
\node[below left]{$O$};
\draw[domain=-3.25:1, smooth, samples=100, very thick]plot(\x-.5, {-.5*(\x+3)*(\x+.5)*(\x+.25)+3});
\draw[dashed](-3.25-.5, 0)node[below]{$a$}--(-3.25-.5,4.03)node[above]{最大};
\draw[dashed](-2.13-.5, 0)node[below]{$x_1$}--(-2.13-.5,1.67)node[below, fill=white]{极小};
\draw[thick](-2.13-.5-.5,1.67)--(-2.13-.5+.5,1.67);
\draw[dashed](-.37-.5, 0)node[below]{$x_2$}--(-0.37-.5,3.03)node[above]{极大};
\draw[thick](-.37-.5-.5,3.03)--(-.37-.5+.5,3.03);
\draw[domain=.5:4.5, smooth, samples=100, very thick]plot(\x/2+.25, {(\x-2)^2-3})node[above]{极大};
\draw[dashed](1.25, 0)node[above]{$x_3$}--(1.25,-3)node[below]{最小};
\draw[thick](1.25-.5,-3)--(1.25+.5,-3);
\draw[dashed](2.5, 0)node[below]{$x_4$}--(2.5,3.25);
\draw[domain=2.5:4.25, smooth, samples=100, very thick]plot(\x, {(\x-3.5)^2+2.25});
\draw[dashed](4.25, 2.81)node[above]{$y=f(x)$}--(4.25,0)node[below]{$b$};
\draw[thick](3.5-.5,2.25)--(3.5+.5,2.25);

\draw[dashed](3.5, 0)node[below]{$x_5$}--(3.5,2.25)node[below, fill=white]{极小};
...
$\therefore\quad$函数$y=\log_a x\; (a>1)$的图像上凸.
\end{proof}

根据定义，证明曲线$y=f(x)$在给定区间内是否上凸（或下凸），一般不像例12.34这么容易，下面我们介绍一种如何利用导数来确定曲线$y=f(x)$凸性的方法.

对于在闭区间$[a,b]$上连续，在开区间内可导的函数$y=f(x)$，其图像如果是下凸的，从图12.16可以看出，曲线$y=f(x)$在点$(x,f(x))$处切线的上方（切点除外），并且随着$x$的增大，各点切线的斜率$f'(x)$也跟着增大. 又因为$f'(x)$的单调性可由$f'(x)$的导数，即$f''(x)$的
正、负来判定. 于是启发我们可用$f(x)$的二阶导数来判定函数$y=f(x)$图像的凸性.
...
大家知道，对于函数$f(x)=\sin x\; (x\in (0,\pi))$, $x=\frac{\pi}{2}$是它的极大值点，$f'(x)$在点$\frac{\pi}{2}$附近的符号是“左正右负”与函数的图像上凸是一致的，因此，又可用$f''(x)$在该点值的正负来判定驻点是函数的极大值点还是极小值点. 下面给出一种利用二阶导数来判定函数极值的方法（证明留给读者）.
...
\begin{thm}
    {定义} 设函数$y=f(x)$的图像上一点$P(x_0,f(x_0))$恰为曲线$y=f(x)$上凸与下凸部分的分界点，则称点$P(x_0, f(x_0))$为曲线$y=f(x)$的拐点，$x_0$称为函数$f(x)$的拐点.
\end{thm}
...
\item $f''(x)$的值在点的两侧异号
\end{enumerate}
则点$P(x_0,f(x_0))$为函数$y=f(x)$图像的拐点.
...
\section{函数的图像绘制}
前面我们对函数的性质进行了进一步的研究，这就使我们能依据函数的性质，更准确地作出函数的图像. 今后在绘制
一个函数的图像之前，一般应依次对函数作如下分析，然后列表画图.
\begin{enumerate}
\item 确定函数的定义域和值域（可大致确定图像的范围）；
\item 验证函数的奇偶性、周期性（可缩小研究函数的范围）；
\item 找出函数图像的某些特殊点（例如图像与坐标轴的交点、间断点及其他特殊点）；
\item 利用导数求出函数的单调区间、极值点和极值、图像的上凸或下凸区间、拐点等；
\item 考察函数的图像是否有渐近线（了解图像若无限延伸时，它的变化趋势）.
\end{enumerate}
...
$\therefore\quad $ 函数 $f(x)$的图像在区间$(-\infty,-1)$内上凸

$\because\quad $ 当$x\in(-1,+\infty)$时, $f''(x)>0$

$\therefore\quad$ 函数 $f(x)$的图像在区间$(-1,+\infty)$内下凸
...
$\therefore\quad $ 函数 $f(x)$的图像的渐近线方程是 $x=-1$ 和
$y=x-1$.
...
\end{tabular}
\end{center}
函数$f(x)=\frac{x^2}{x+1}$的图像如图12.17.
\begin{figure}[htp]
    \centering
...
\subsection{导数的应用}
作为导数的应用，在本章中我们利用一阶、二阶导数讨论了函数的单调性、极值、最值、拐点及函数的图像的凸性等.

利用导数研究函数性质，使我们能够较为准确地作出函数的图像.
