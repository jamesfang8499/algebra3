...
由于$f(x)=\frac{x+1}{x}=1+\frac{1}{x}$，观察函数$y=f(x)=\frac{x+1}{x}$的图像（如图11.1）可以看出：
\begin{figure}[htp]
    \centering
...
\item 作函数$y=\frac{-x}{x+2}$的图像（如图11.4）.
...
$\therefore\quad$ 观察函数 $y=\frac{x^{2}-1}{x-1}$ 的图像（如图 11.5）可知, 此函数图像是一条在点 $A(1,2)$处有个“洞”的直线.
...
现在我们看函数$f(x)=\frac{x+1}{x}$在点$x=0$处附近的变化趋势. 由函数$y=\frac{x+1}{x}$的图像(如图 11.1)可知：
\begin{itemize}
...
\noindent
\begin{minipage}{.45\textwidth}
\item 已知：$f(x)=\arccos x$, 根据函数的极限的定义和函数的图像, 写 出 下 列 各 函 数 指 定 的 极 限 . 
\begin{multicols}{2}
\begin{enumerate}[(1)]
...
-x+1 & x<0
\end{cases}$，
根据函数的极限的定义和函数的图像, 写 出 下 列 各 函 数 指 定 的 极 限 . 
\begin{multicols}{2}
\begin{enumerate}[(1)]
    \item $\lim\limits _{x\to 0^+ }f( x)$
    \item $\lim\limits _{x\to 0^- }f( x)$
...
\begin{enumerate}
    \item 根据函数的极限的定义及函数的图像，分别写出下列各极限：
\begin{multicols}{3}
\begin{enumerate}[(1)]
...
\item 已知：$f(x)=\begin{cases}
    ...
    0& x=0\\
    -(x+1)^2 & x<0
\end{cases}$，作函数$y=f(x)$的图像，写出下列各极限：
\begin{multicols}{4}
\begin{enumerate}[(1)]
    \item $\Lim{x}{1}f(x)$
    \item $\Lim{x}{-1}f(x)$
...
\begin{ex}
\begin{enumerate}
    \item 已知函数$y=f(x)$的图像如下图所示，请将符合下述性质的函数图像的代号A、B、C、D分别填在指定位置。
\begin{enumerate}[(1)]
...
\begin{thm}{性质4}
    若函数$y=f(x)$在区间$(a,b)$内连续，则函数$y=f(x)$在区间$(a,b)$内的图像是一条连续曲线.
\end{thm}
...
观察例11.11中3个函数的图像，我们发现，第（1）、（3）个函数在给定的闭区间上都存在着最大值和最小值，而第（2）个函
数没有最大值和最小值.
...
观察例11.11中3个函数的图像我们还可以发现，当自变量$x$的值在给定区间内变化时，若使对应的函数值$f(x)$由负变正（或由正变负），则在它们中间一定存在一个点$x_0$，使得$f(x_0)=0$.
...
\begin{enumerate}[(1)]
    ...
    \item \[\begin{split}
        \Lim{x}{0}\frac{1-\cos 2x}{x^2}&=\Lim{x}{0}\frac{2\sin^2 x}{x^2}=2\Lim{x}{0}\left(\frac{\sin x}{x}\right)^2=2\left(\Lim{x}{0}\frac{\sin x}{x}\right)^2\\
        &=2\x 1^2=2
    \end{split}\]
\end{enumerate}
\end{solution}
...
\begin{enumerate}[(1)]
    \item \[\begin{split}
\text{原式}= \lim_{x\to 0}\frac{2\cos\left(\frac{\pi}{3}+\frac{x}{2}\right)\sin\frac{x}{2}}{x}&= \lim_{x\to 0}\left[\frac{\sin\frac{x}{2}}{\frac{x}{2}}\cdot \cos\left(\frac{\pi}{3}+\frac{x}{2}\right)\right]\\
